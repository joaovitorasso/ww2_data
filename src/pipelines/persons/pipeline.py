...
try:
    ...
    from src.load.persons import get_existing_details, get_output_path, import_details_to_db, load_transformed_data
    from src.pipelines.countries.pipeline import run as run_countries_pipeline
    from src.transform.persons import parse_person_details, transform_person_sections
    ...
    from load.persons import get_existing_details, get_output_path, import_details_to_db, load_transformed_data
    from pipelines.countries.pipeline import run as run_countries_pipeline
    from transform.persons import parse_person_details, transform_person_sections
...
def _process_person_retry_queue() -> None:
    wait_seconds, max_rows = get_retry_queue_settings()
    pending = get_retry_persons(limit=max_rows)
    if not pending:
        print("No pending 429 retries for persons.")
        return

    print(f"Waiting {wait_seconds}s before retrying {len(pending)} persons from retry queue...")
    if wait_seconds > 0:
    ...
    for row in pending:
        ...
        person_link = basic.get("person_link") or basic.get("link")

        if not alliance or not person_link:
            mark_retry_person_failed(retry_id, "missing alliance or person_link in retry payload")
            failed += 1
            continue

        try:
            details = asdict(parse_person_details(person_link))
            raw_payload = {"basic": basic, "details": details}
            raw_id = queue_raw_id or insert_raw_person(raw_payload, alliance)
            bronze_id = insert_bronze_person(raw_id, raw_payload, alliance)
            insert_silver_person(bronze_id, raw_payload, alliance)
            mark_retry_person_succeeded(retry_id)
            succeeded += 1
        except Exception as exc:
            mark_retry_person_failed(retry_id, str(exc))
            failed += 1

    remaining = len(get_retry_persons(limit=max_rows))
    print(f"Persons retry queue finished. success={succeeded}, failed={failed}, remaining={remaining}")
...
def run(limit: int | None = None) -> None:
    ...
    init_db()
    execution_id = start_pipeline_execution("persons")
    ...
    try:
        ...
        raw = extract_persons()
        data = transform_person_sections(raw, limit=limit)
