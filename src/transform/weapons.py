...
def parse_weapon_details_from_html(html: str, link: str) -> Weapon:
    ...
    img_tag = (
        ...
        or soup.find("img", src=lambda value: value and "weapon" in value.lower() and "/images/" in value)
        or soup.find("img", src=lambda value: value and "/images/" in value and "weapon" in value.lower())
    )
    if img_tag:
        img_src = (
...
def transform_weapon_sections(weapon_sections: dict[str, list[dict]], limit: int | None = None) -> dict[str, list[dict]]:
    remaining = limit
    if limit:
    ...
    for alliance, weapons in weapon_sections.items():
        ...
        for entry in weapons:
            ...
            try:
                ...
                detailed_weapons.append({"basic": entry, "details": details_cache[weapon_link]})
            except Exception as exc:
                detailed_weapons.append({"basic": entry, "details": None, "error": str(exc)})
            finally:
